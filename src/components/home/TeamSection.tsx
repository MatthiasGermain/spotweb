...
// N'affiche un lien que s'il est réellement renseigné (pas "#" ni vide).
...
function SocialLinks({
  ...
  const hasLinkedin = isValidLink(member.linkedin);
  const hasInstagram = isValidLink(member.instagram);
    ...
    >
      ...
      {isMobile && isExpanded && (
        <div className="absolute inset-x-0 bottom-0 bg-[#628f93]/90 p-4 animate-slide-up">
          ...
          <SocialLinks member={member} stopPropagation />
        </div>
      )}
