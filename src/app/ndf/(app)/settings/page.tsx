...
import { ensureDefaultAssociations, getAssociations, getAssociationById } from "@/lib/ndf/associations";
import ConfirmForm from "@/components/ndf/ConfirmForm";
import { saveAssociationAction, deleteLogoAction, deleteAssociationAction } from "./actions";
    ...
    <div className="page-sm">
      ...
      {!showForm ? (
        <div className="card">
          ...
          {assocs.length === 0 ? (
            <div className="card-body text-center py-8" style={{ color: "var(--muted-foreground)" }}>
              Aucune association configurée.
            </div>
          ) : (
            <div>
              {assocs.map((a) => (
                <div key={a.id} className="flex items-center gap-4 p-4" style={{ borderTop: "1px solid var(--border)" }}>
                  {a.logoUrl ? (
                    ...
                    <div
                      ...
                      style={{ border: "1px dashed var(--border)", background: "var(--muted)", color: "var(--muted-foreground)" }}
                    >
                      Logo
                    </div>
                  )}
                  ...
                  <div className="flex gap-2 shrink-0">
                    <a href={`/ndf/settings?edit=${encodeURIComponent(a.id)}`} className="btn btn-secondary btn-sm">
                      ✏ Modifier
                    </a>
                    <ConfirmForm action={deleteAssociationAction} confirmMessage={`Supprimer « ${a.nom} » ?`}>
                      <input type="hidden" name="assoc_id" value={a.id} />
                      <button type="submit" className="btn btn-sm" style={{ borderColor: "#fca5a5", color: "#dc2626" }}>
                        ✕
                      </button>
                    </ConfirmForm>
                  </div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="card-title">{isNew ? "Nouvelle association" : `Modifier : ${editAssoc?.nom}`}</div>
          </div>
          <div className="card-body">
            <form action={saveAssociationAction} encType="multipart/form-data" className="field-group">
              ...
              <div className="field">
                ...
                <input className="input" type="text" name="nom" defaultValue={editAssoc?.nom ?? ""} placeholder="ex : Eglise Connexion" required />
              </div>
              <div className="field">
                ...
                />
                <p className="field-description">Affichée sous le nom dans l&apos;en-tête du PDF.</p>
              </div>
              <div className="field">
                <label className="field-label">Email du trésorier</label>
                <input className="input" type="email" name="email" defaultValue={editAssoc?.email ?? ""} placeholder="tresorier@monasso.fr" />
              ...
              <div className="field">
                ...
                <div className="drop-zone">
                  <label style={{ cursor: "pointer", display: "block" }}>
                    ...
                    <p className="text-sm font-medium">{editAssoc?.logoUrl ? "Remplacer le logo" : "Choisir un logo"}</p>
